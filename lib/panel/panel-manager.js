...
import InformationView from './../info/information-view'
import GodocPanel from './../doc/godoc-panel'
import GodocView from './../doc/godoc-view'
...
class PanelManager {
  ...
  refreshPanel (panelOrientation) {
    ...
    if (this.panelOrientation === 'horizontal') {
      this.panel = atom.workspace.addBottomPanel({item: this.goPlusPanel, visible: this.panelVisible, priority: -1000})
    } else {
  ...
  registerOpeners () {
    this.subscriptions.add(atom.workspace.addOpener(uri => {
      if (uri === 'atom://go-plus-information') {
        console.log(this.goconfig)
        const model = new Information(this.goconfig)
        return new InformationView({model: model})
      }
    }))

    this.subscriptions.add(atom.workspace.addOpener(uri => {
      if (uri === 'atom://go-plus-godoc') {
        console.log(this.goconfig)
        const model = new GodocPanel()
        return new GodocView({model: model})
      }
    }))
  }
  ...
  subscribeToCommands () {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', 'golang:toggle-info', () => {
      atom.workspace.toggle('atom://go-plus-information')
    }))
    this.subscriptions.add(atom.commands.add('atom-workspace', 'golang:toggle-godoc', () => {
      atom.workspace.toggle('atom://go-plus-godoc')
    }))
