...
class GocodeProvider {
          ...
          .then((rawSuggestions) => {
            if (rawSuggestions.length === 0 && prefix === '.') {
              return isVendorSupported(this.goconfig).then((useVendor) => {
                const pkg = this.wantedPackage(buffer, bufferPosition)
                if (!pkg) {
                  return []
                }
                const pkgPath = this.findPackage(file, pkg, useVendor)
                if (!pkgPath) {
                  return []
                }
                const added = this.addImport(buffer, pkgPath, offset)
                if (!added) {
                  return []
                }
  ...
  findPackage (file, pkg, useVendor) {
    let pkgs = this.allPkgs.get(pkg)
    if (!pkgs) {
      return null
    }

    const vendor = path.sep + 'vendor' + path.sep
    if (useVendor) {
      // find the closest vendored package by walking up the dir tree
      let dir = path.dirname(file)
      const vendorPkgs = pkgs.filter((pkg) => pkg.lastIndexOf(vendor) > 0)
      if (vendorPkgs.length) {
        const stopAt = ['', path.sep, '.']
        while (!stopAt.includes(dir)) {
          // note: this takes the first match it finds, even though multiple packages might fit...
          const pkg = vendorPkgs.find((pkg) => {
            const p = pkg.substr(0, pkg.lastIndexOf(vendor))
            return dir.endsWith(path.normalize(p))
          })
          if (pkg) {
            return pkg
          }
    ...
    return pkgs.find((pkg) => pkg.lastIndexOf(vendor) === -1)
  }

  addImport (buffer, pkg, offset) {
