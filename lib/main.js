...
export default {
  ...
  loadBuilder (linter) {
    ...
    this.builder = new Builder(this.provideGoConfig(), () => this.buildLinter)
  ...
  consumeLinter (registry) {
    this.buildLinter = registry.register({name: 'go build'})
    this.subscriptions.add(this.buildLinter)

    this.gometalinterLinter = registry.register({name: 'gometalinter'})
  ...
  deserializeInformationView (serialized) {
    const {Information} = require('./info/information')
    const model = new Information(this.provideGoConfig())
    const InformationView = require('./info/information-view')
    return new InformationView({model: model})
  },

  deserializeGodocView (serialized) {
    this.loadDoc()
    const GodocView = require('./doc/godoc-view')
    const model = this.godoc.getPanel()
